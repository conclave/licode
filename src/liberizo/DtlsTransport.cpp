    ...
    , running_(false)
{
  ...
  dtlsRtp_.reset(new DtlsSocketContext());
  ...
  (new DtlsFactory())->createClient(dtlsRtp_);
  dtlsRtp_->setDtlsReceiver(this);

  int comps = 1;
  if (!rtcp_mux) {
    comps = 2;
    dtlsRtcp_.reset(new DtlsSocketContext());
    (new DtlsFactory())->createClient(dtlsRtcp_);
    dtlsRtcp_->setDtlsReceiver(this);
  }
...
{
  return dtlsRtp_->getFingerprint();
}
...
void DtlsTransport::processLocalSdp(SdpInfo* sdp)
{
  ELOG_DEBUG("Processing Local SDP in DTLS Transport");
  sdp->isFingerprint = true;
  sdp->fingerprint = getMyFingerprint();
  std::string username;
  std::string password;
